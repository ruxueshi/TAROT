...
def predict(clf, 
    ...
    pred_list, loss_list = [], []
    x_test = clf.input_encoder.feature_extractor(x_test, target, table_flag=table_flag)
    ...
    for i in range(0, x_len, eval_batch_size):
        ...
        with torch.no_grad():
            logits = clf(bs_x_test, [target], adj_list)
            loss = compute_loss(criterion, logits, y_test, 2)
