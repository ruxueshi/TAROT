...
from .modeling_TNE import CoDTabFeatureExtractor_graph
...
from utils_llm import compute_loss
...
class Trainer:
    def __init__(self,
        ...
        **kwargs,
        ...
        self.feature_extractor = CoDTabFeatureExtractor_graph(
            categorical_columns=model.categorical_columns,
            numerical_columns=model.numerical_columns,
            binary_columns=model.binary_columns,
        ...
        for dataindex, trainset in enumerate(train_set_list):
            ...
            if trainset[1] is not None:
                ...
                y = None
            inputs = self.feature_extractor(x, target, table_flag=dataindex)
        ...
        for epoch in trange(args['num_epoch'], desc='Epoch'):
            ...
            for dataindex in range(len(self.trainloader_list)):
                ...
                for data in self.trainloader_list[dataindex]:
                    ...
                    logits = self.model(data[0], [self.target], self.adj_list)
                    loss = compute_loss(criterion, logits, data[1], 2)
            ...
            if self.test_set_list is not None:
                ...
                if self.early_stopping(-eval_res, self.model) and eval_data:
                    if self.regression_task:
                        ypred = predict(self.model, eval_data[0], self.target, self.adj_list, regression_task=True)
                        ans = evaluate(ypred, eval_data[1], metric='rmse')
                    else:
                        ypred = predict(self.model, eval_data[0], self.target, self.adj_list)
    ...
    def train_no_dataloader(self,
        ...
        for epoch in range(args['num_epoch']):
            ...
            if self.test_set is not None:
                ...
                pred_all = predict(self.model, x_test, self.target, self.adj_list, self.args['eval_batch_size'])
                eval_res = self.args['eval_metric'](y_test, pred_all)
