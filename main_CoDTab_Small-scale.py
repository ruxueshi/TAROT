...
for i in range(args.graph_num):
    adj = np.load(fr"data\graph{i+1}\{args.data}_{args.seed}_{args.shot}_graph.npy")
...
def train_adapter(X_train_now, label_list, shot, y_train_num):
    ...
    for _ in range(1000):
        opt.zero_grad()
        outputs = model(X_train_now, [target], adj_list)
...
for i in range(0, x_len, eval_batch_size):
    test_outputs = trained_model(X_test[i:i+eval_batch_size], [target], adj_list)
...
with open(f"result/{args.data}/result_{args.shot}shot_{args.layer}layer_{args.seed}seed.txt", "w", encoding="utf-8") as f:
